...
def init_redis():
    global members, songs, user_song, room
    init_redis_common()
    init_redis_room()

    members = RedisProxy(redis_room, ROOM_MEMBER_KEY, 'fullname', ['fullname', 'nick', 'avatar'])
    songs = ListRedisProxy(redis_room, ROOM_SONG_KEY, 'fullname', ['sid', 'name', 'author', 'nick', 'fullname', 'duration'])
...
class HashRedisProxy(RedisProxy):
    ...
    status = {'status': RoomStatus.singing, "start_time": 0, "end_time": 0, "current_time": 0, "duration": 0, "room": ""}
