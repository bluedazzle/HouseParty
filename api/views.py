...
class SongListView(CheckSecurityMixin, StatusWrapMixin, MultipleJsonResponseMixin, ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if query:
            fuzzy_queryset = Song.objects.raw(
                '''SELECT * FROM core_song WHERE to_tsvector('parser_name', name) @@ plainto_tsquery('parser_name', '{0}') or to_tsvector('parser_name', author) @@ plainto_tsquery('parser_name', '{1}') ORDER BY author desc;'''.format(
                    query, query))[0:200]
            self.raw_count = len(list(queryset))
            self.paginator_class = SearchPaginator
            match_queryset = Song.objects.filter(Q(name=query) | Q(author=query)).all()
            match_queryset = []
            res_list = [itm for itm in match_queryset]
            res_list.extend(fuzzy_queryset)
            queryset = list(tuple(res_list))
    ...
    def get_queryset(self):
        queryset = super(SongListView, self).get_queryset().filter(hidden=False).order_by('-recommand')[0:500]
        return queryset
...
class KickView(CheckTokenMixin, StatusWrapMixin, JsonResponseMixin, DetailView):
    model = PartyUser
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not fullname:
            ...
            self.status_code = ERROR_DATA
            return self.render_to_response({})
        users = self.model.objects.filter(fullname=fullname)
        if users.exists():
            user = users[0]
            singers = Singer.objects.filter(creator=user, room=user.room)
            if singers.exists():
                singer = singers[0]
                singer.delete()
            user.room = None
...
class UserInfoListView(StatusWrapMixin, MultipleJsonResponseMixin, ListView):
    model = PartyUser
    exclude_attr = ['password', 'token', 'qq_open_id', 'forbid', 'wx_open_id', 'online', 'room_id']
    http_method_names = ['post']
    paginate_by = 50

    def get_queryset(self):
        data = json.loads(self.request.body)
        queryset = self.model.objects.filter(fullname__in=data)
        return queryset
