    ...
    def get(self, request, *args, **kwargs):
        ...
        if query:
            fuzzy_queryset = Song.objects.raw(
                '''SELECT * FROM core_song WHERE to_tsvector('parser_name', name) @@ plainto_tsquery('parser_name', '{0}') or to_tsvector('parser_name', author) @@ plainto_tsquery('parser_name', '{1}');'''.format(
                    query, query))[0:200]
